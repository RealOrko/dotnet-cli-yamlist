...
namespace yamlist.Modules.Commands.Options
{
    ...
    {
        public static object Parse(Type commandType, Type argumentsType, string[] args,
            Func<string, string> getEnvironmentVariable = null)
        {
            ...
            {
                var attribute = propertyInfo.GetCustomAttributes(typeof(ArgumentAttribute), false)
                    .Cast<ArgumentAttribute>().FirstOrDefault();
                if (attribute != null)
                {
                    availableCommandArguments.AddRange(new List<string>
                        {attribute.LongName?.ToLowerInvariant(), attribute.ShortName?.ToLowerInvariant()});
                    if (ParseArgumentValue(attribute, target, propertyInfo, args)) continue;
        ...
        private static void ValidateArguments(Type commandType, Type argumentsType, string[] args,
            List<string> availableCommandArguments)
        {
        ...
        public static T Parse<T>(Type commandType, string[] args, Func<string, string> getEnvironmentVariable = null)
            where T : new()
        {
            ...
            Func<string, string> getEnvironmentVariable = null)
        {
            ...
            if (!string.IsNullOrEmpty(attribute.Default)) results.Add(attribute.Default);

            return results;
        }

        private static bool ParseArgumentValue(ArgumentAttribute attribute, object target, PropertyInfo propertyInfo,
            string[] args)
        {
            for (var index = 0; index < args.Length; index++)
            {
                ...
                if (!arg.Equals(attribute.ShortName) && !arg.Equals(attribute.LongName)) continue;
